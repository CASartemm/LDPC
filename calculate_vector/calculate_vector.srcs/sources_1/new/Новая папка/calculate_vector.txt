...
reg prev_state;        // Предыдущее состояние для отслеживания перехода

// Определение состояний
localparam RECEIVING = 1'b0;    // Прием входных битов
localparam TRANSMITTING = 1'b1; // Передача результата

// Новый сигнал сброса для shift_register_processor
...
// Инстанцирование shift_register_processor с новым сигналом сброса
...
always @(posedge aclk or negedge aresetn) begin
    if (!aresetn) begin
        ...
        case (state)
            RECEIVING: begin
                int_m_axis_tready <= m_axis_tready; // Связываем с внешним tready
