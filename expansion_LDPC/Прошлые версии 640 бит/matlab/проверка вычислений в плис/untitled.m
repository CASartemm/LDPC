...
if size(Pg, 1) ~= 640 || size(Pg, 2) ~= 128
...
bit_vector_str = fscanf(fileID, '%s');
fclose(fileID);
bit_vector = double(bit_vector_str - '0');  % Убрали переворот
if length(bit_vector) ~= 640
    error('Входной вектор должен содержать 640 бит');
end
